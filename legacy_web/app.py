...
def create_app(store, rebalancer, config) -> Flask:
    ...
    def positions_page():
        ...
        # Read-only here; doesn't displace the intent-based table below
        # (that migration is P5).  Surfaces three things the intent table
        # can't: pending redemptions (chain has winning shares we forgot
        # to redeem), drift (intent ≠ chain — phantom-closed positions
        # like the 2026-05-04 Chicago case), and snapshot freshness.
        snapshot_rows = _run_async(st.get_wallet_snapshot())
        snapshot_meta = _run_async(st.get_wallet_snapshot_meta())
        # Partition snapshot rows for the three new sections.
        pending_redemptions = [
            r for r in snapshot_rows
            if r.get("redeemable") and (r.get("shares") or 0) > 0.005
        ]
        drift_rows = [
            r for r in snapshot_rows
            if abs(r.get("drift_shares") or 0) > 0.005
        ]
        # Build a chain-truth lookup keyed by token_id so the existing
        # positions table can show a "chain shares" column alongside
        # the intent shares.  This lets the operator spot per-row drift
        # without scrolling to the dedicated section.
        snapshot_by_token = {r["token_id"]: r for r in snapshot_rows}
        active_keys = active_variant_keys()
        active_set = set(active_keys)
        ...
        # Per-city trim ratio for the dashboard column.  Pulls the
        # runtime-effective ratio (per-city dict if city is keyed, else
        # global fallback) so the operator can see at a glance how
        # tightly each held position is gated against price drawdown.
        # See StrategyConfig.trim_price_stop_ratio_per_city.
        per_city_trim = cfg.strategy.trim_price_stop_ratio_per_city or {}
        global_trim = cfg.strategy.trim_price_stop_ratio
        for p in open_pos:
            ...
            # Trim ratio + source.  ``source`` distinguishes the per-city
            # override from the global fallback so the UI can render them
            # differently (e.g. dim the global, highlight per-city).
            city = p.get("city", "")
            if city in per_city_trim:
                p["trim_ratio"] = per_city_trim[city]
                p["trim_ratio_source"] = "per_city"
            else:
                p["trim_ratio"] = global_trim
                p["trim_ratio_source"] = "global"
            # Pre-compute the effective stop price so the row shows the
            # absolute level the bot will trim at, not just the ratio.
            try:
                p["trim_stop_price"] = round(
                    effective_entry * (1.0 - p["trim_ratio"]), 4,
                )
            except (TypeError, ValueError):
                p["trim_stop_price"] = None
            # P2 (2026-05-04): per-row drift indicator from snapshot.
            # When intent shares ≠ chain shares, surface the gap so the
            # operator can sanity-check before reading the dedicated
            # "Drift" section.  ``snap_shares`` may be missing if the
            # snapshot hasn't run yet (cold start) — render as None.
            snap_row = snapshot_by_token.get(p["token_id"])
            p["snap_shares"] = snap_row["shares"] if snap_row else None
            p["snap_drift"] = snap_row["drift_shares"] if snap_row else None
        ...
        return render_template(
            ...
            strategy_summary=strategy_summary,
            strat_meta=strat_meta(),
            # P2 (2026-05-04): wallet snapshot — chain-truth view
    ...
    def config_page():
        cfg = app.config["bot_config"]
        # P2-followup (2026-05-03): expose per-city trim_price_stop_ratio
        # alongside the latest calibrator output for each city.  ``source``
        # tells the user whether runtime is using the per-city dict
        # ("per_city") or the global fallback ("global").
        per_city_dict = cfg.strategy.trim_price_stop_ratio_per_city or {}
        global_ratio = cfg.strategy.trim_price_stop_ratio
        # Latest calibrator row per city (best-effort — table is empty
        # until the cron has fired at least once OR the backfill script
        # has run).  Uses the existing bot_store + _run_async helper so
        # we share the same async loop / connection plumbing the rest of
        # the dashboard uses.
        calib_by_city: dict[str, dict] = {}
        try:
            store = app.config.get("bot_store")
            if store is not None and hasattr(store, "list_trim_ratio_calibrations"):
                rows = _run_async(store.list_trim_ratio_calibrations()) or []
                for r in rows:
                    # list is DESC by computed_at — keep the first row
                    # we see per city.
                    calib_by_city.setdefault(r["city"], r)
        except Exception:
            calib_by_city = {}
        # Build display rows in city order (cities the bot tracks first,
        # plus any extras the dict / calibrator added).
        bot_cities = [c.name for c in cfg.cities]
        extra_cities = sorted(
            (set(per_city_dict) | set(calib_by_city)) - set(bot_cities)
        )
        trim_ratios = []
        for city in bot_cities + extra_cities:
            in_dict = city in per_city_dict
            ratio = per_city_dict[city] if in_dict else global_ratio
            calib = calib_by_city.get(city) or {}
            trim_ratios.append({
                "city": city,
                "source": "per_city" if in_dict else "global",
                "ratio": ratio,
                "calib_ratio": calib.get("ratio"),
                "n_episodes": calib.get("n_episodes"),
                "boundary_x": calib.get("boundary_x"),
                "method": calib.get("method"),
                "computed_at": calib.get("computed_at"),
            })

        # N-4 (2026-05-04): surface live-calibrator state on the
        # config page so ops can spot bias drift / verify auto-switch
        # progress.  Pulled directly from rebalancer._error_dists,
        # which the live_calibrator_refresh cron updates daily.
        calibrator_rows: list[dict] = []
        try:
            reb = app.config.get("bot_rebalancer")
            if reb is not None and getattr(reb, "_error_dists", None):
                from src.strategy.live_calibrator import HybridErrorDistribution
                # City order: bot's configured cities first, then any
                # extras (e.g. live-only cities that crept in).
                bot_city_names = [c.name for c in cfg.cities]
                seen: set[str] = set()
                ordered_cities: list[str] = []
                for cn in bot_city_names + list(reb._error_dists.keys()):
                    if cn in seen or cn not in reb._error_dists:
                        continue
                    seen.add(cn)
                    ordered_cities.append(cn)
                for city in ordered_cities:
                    d = reb._error_dists[city]
                    if isinstance(d, HybridErrorDistribution):
                        source = d.source
                        n_live = d._n_live
                        n_cached = d._n_cached
                    else:
                        source = "cached"
                        n_live = 0
                        n_cached = getattr(d, "_count", 0)
                    calibrator_rows.append({
                        "city": city,
                        "source": source,
                        "n_live": n_live,
                        "n_cached": n_cached,
                        "mean": getattr(d, "mean", 0.0),
                        "std": getattr(d, "std", 0.0),
                    })
        except Exception:
            calibrator_rows = []
    ...
    def api_status():
        ...
        # P1 (2026-05-04): expose snapshot freshness so external monitoring
        # (quick_status.sh, hourly_monitor) can detect stalled snapshot
        # refresh independently of the bot's main loop heartbeat.
        snapshot_meta = _cached("snapshot_meta", ttl=10)
        if snapshot_meta is None:
            snapshot_meta = _run_async(
                app.config["bot_store"].get_wallet_snapshot_meta(),
            )
            _set_cache("snapshot_meta", snapshot_meta)
        ...
        return jsonify({
            ...
            "trends": state.get("trends", {}),
            "wallet_snapshot": {
                "last_refresh_at": (snapshot_meta or {}).get("last_refresh_at"),
                "status": (snapshot_meta or {}).get("last_refresh_status"),
                "tokens_seen": (snapshot_meta or {}).get("tokens_seen", 0),
                "drift_count": (snapshot_meta or {}).get("drift_count", 0),
                "pending_redemption_count": (snapshot_meta or {}).get(
                    "pending_redemption_count", 0,
                ),
                "duration_ms": (snapshot_meta or {}).get("duration_ms", 0),
                "error": (snapshot_meta or {}).get("last_refresh_error"),
